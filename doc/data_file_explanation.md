...
* ***{take ID}/***: take folder named by the take id, e.g. 000000.
  * ***raw/***: raw data directly exported by each recording device. Note that the raw data **not used** in the processing is highlighted by the <u>underline</u>.
    * ***hand/***: hand pose data generated by [Hand Engine](https://stretchsense.com/solution/hand-engine/). 
      * **P1L.csv** / **P1R.csv**: sensor reading and hand pose data of left (L) and right (R) hand.  
        ...
        * index 00-03, middle 00-03, pinky 00-03, ring 00-03, thumb 01-03: hand joint pose in eular-angle format (xyz degrees). Each joint pose is the relative rotation with respect to its parent in the kinematic tree. Please refer to [our paper]() for the detail of the used hand model and its articulated links.  
    ...
    * **optitrack.csv**: object tracking information recorded by [OptiTrack](https://optitrack.com/).
      * object id: the id of the tracked object. 115 is the helmet. 116 is the right hand tracking plate. 117 is the left hand tracking plate. 118 is the headband. 
  ...
  * ***processed/***: formatted data derived from raw data. 
    ...
    * **alignment.json**: a dictionary whose keys represent the frame indices. The corresponding value for each key is a mapping between stream id and its timestamp. In each frame (key), we align each stream timestamp to the frame reference timestamp (`rgbd0`) by finding the closest one to the reference.
