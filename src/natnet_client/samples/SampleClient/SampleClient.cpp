...
Usage [optional]:
        ...
        [ServerIP]			IP address of the server (e.g. 192.168.0.107) ( defaults to
local machine) [OutputFilename]	Name of points file (pts) to write out.  defaults to
Client-output.pts
...
void NATNET_CALLCONV ServerDiscoveredCallback(const sNatNetDiscoveredServer* pDiscoveredServer,
                                              void* pUserContext);
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data,
                                 void* pUserData);  // receives data from the server
void NATNET_CALLCONV MessageHandler(Verbosity msgType,
                                    const char* msg);  // receives NatNet error messages
void resetClient();
int ConnectClient();
...
int main(int argc, char* argv[]) {
  ...
  g_pClient->SetFrameReceivedCallback(
      DataHandler, g_pClient);  // this function will receive data from the server
  ...
  if (argc == 1) {
    ...
    printf(
        "Press the number key that corresponds to any discovered server to connect to that "
        "server.\n");
    ...
    for (int i = 0; i < pDataDefs->nDataDescriptions; i++) {
      ...
      if (pDataDefs->arrDataDescriptions[i].type == Descriptor_MarkerSet) {
        ...
        printf("Force Plate Electrical Center Offset (%3.3f, %3.3f, %3.3f)\n", pFP->fOriginX,
               pFP->fOriginY, pFP->fOriginZ);
        for (int iCorner = 0; iCorner < 4; iCorner++)
          printf("Force Plate Corner %d : (%3.4f, %3.4f, %3.4f)\n", iCorner,
                 pFP->fCorners[iCorner][0], pFP->fCorners[iCorner][1], pFP->fCorners[iCorner][2]);
  ...
  while (c = getch()) {
    switch (c) {
      ...
      case 'd':  // disconnect
        ...
        // to that client endpoint
        iResult = g_pClient->SendMessageAndWait("Disconnect", &response, &nBytes);
        if (iResult == ErrorCode_OK) printf("[SampleClient] Disconnected");
        break;
      default:
        break;
    }
...
void NATNET_CALLCONV ServerDiscoveredCallback(const sNatNetDiscoveredServer* pDiscoveredServer,
  ...
  if (pDiscoveredServer->serverDescription.bConnectionInfoValid == false) {
    warning =
        " (WARNING: Legacy server, could not autodetect settings. Auto-connect may not work "
        "reliably.)";
  }

  printf("[%c] %s %d.%d at %s%s\n", serverHotkey, pDiscoveredServer->serverDescription.szHostApp,
         pDiscoveredServer->serverDescription.HostAppVersion[0],
         pDiscoveredServer->serverDescription.HostAppVersion[1], pDiscoveredServer->serverAddress,
         warning);
...
int ConnectClient() {
  ...
  if (retCode != ErrorCode_OK) {
    ...
    printf("Application: %s (ver. %d.%d.%d.%d)\n", g_serverDescription.szHostApp,
           g_serverDescription.HostAppVersion[0], g_serverDescription.HostAppVersion[1],
           g_serverDescription.HostAppVersion[2], g_serverDescription.HostAppVersion[3]);
    printf("NatNet Version: %d.%d.%d.%d\n", g_serverDescription.NatNetVersion[0],
           g_serverDescription.NatNetVersion[1], g_serverDescription.NatNetVersion[2],
...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data, void* pUserData) {
  ...
  // and
  //   b) The time immediately prior to the NatNet frame being transmitted over the network
  //   (TransmitTimestamp)
  //
  // This figure may appear slightly higher than the "software latency" reported in the Motive user
  // interface, because it additionally includes the time spent preparing to stream the data via
  // NatNet.
  ...
  // Only recent versions of the Motive software in combination with ethernet camera systems support
  // system latency measurement. If it's unavailable (for example, with USB camera systems, or
  // during playback), this field will be zero.
  const bool bSystemLatencyAvailable = data->CameraMidExposureTimestamp != 0;

  if (bSystemLatencyAvailable) {
    // System latency here is defined as the span of time between:
    //   a) The midpoint of the camera exposure window, and therefore the average age of the photons
    ...
    // Client latency is defined as the sum of system latency and the transit time taken to relay
    // the data to the NatNet client. This is the all-inclusive measurement (photons to client
    // processing).
    const double clientLatencyMillisec =
        pClient->SecondsSinceHostTimestamp(data->CameraMidExposureTimestamp) * 1000.0;

    // You could equivalently do the following (not accounting for time elapsed since we calculated
    // transit latency above):
  ...
  NatNet_DecodeTimecode(data->Timecode, data->TimecodeSubframe, &hour, &minute, &second, &frame,
                        &subframe);
  ...
  for (i = 0; i < data->nRigidBodies; i++) {
    ...
    printf("Rigid Body [ID=%d  Error=%3.2f  Valid=%d]\n", data->RigidBodies[i].ID,
           data->RigidBodies[i].MeanError, bTrackingValid);
    printf("\tx\ty\tz\tqx\tqy\tqz\tqw\n");
    printf("\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\n", data->RigidBodies[i].x,
           data->RigidBodies[i].y, data->RigidBodies[i].z, data->RigidBodies[i].qx,
           data->RigidBodies[i].qy, data->RigidBodies[i].qz, data->RigidBodies[i].qw);
  }
  ...
  for (i = 0; i < data->nSkeletons; i++) {
    ...
    for (int j = 0; j < skData.nRigidBodies; j++) {
      ...
      printf("Bone %d\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\n", rbData.ID, rbData.x,
             rbData.y, rbData.z, rbData.qx, rbData.qy, rbData.qz, rbData.qw);
    }
  }

  // labeled markers - this includes all markers (Active, Passive, and 'unlabeled' (markers with no
  // asset but a PointCloud ID)
  bool bOccluded;      // marker was not visible (occluded) in this frame
  bool bPCSolved;      // reported position provided by point cloud solve
  bool bModelSolved;   // reported position provided by model solve
  bool bHasModel;      // marker has an associated asset in the data stream
  bool bUnlabeled;     // marker is 'unlabeled', but has a point cloud ID that matches Motive
                       // PointCloud ID (In Motive 3D View)
  ...
  for (i = 0; i < data->nLabeledMarkers; i++) {
    ...
    printf(
        "%s Marker [ModelID=%d, MarkerID=%d, Occluded=%d, PCSolved=%d, ModelSolved=%d] "
        "[size=%3.2f] [pos=%3.2f,%3.2f,%3.2f]\n",
        szMarkerType, modelID, markerID, bOccluded, bPCSolved, bModelSolved, marker.size, marker.x,
        marker.y, marker.z);
  }
  ...
  for (int iPlate = 0; iPlate < data->nForcePlates; iPlate++) {
    ...
    for (int iChannel = 0; iChannel < data->ForcePlates[iPlate].nChannels; iChannel++) {
      ...
      for (int iSample = 0; iSample < data->ForcePlates[iPlate].ChannelData[iChannel].nFrames;
           iSample++)
  ...
  for (int iDevice = 0; iDevice < data->nDevices; iDevice++) {
    ...
    for (int iChannel = 0; iChannel < data->Devices[iDevice].nChannels; iChannel++) {
      ...
      for (int iSample = 0; iSample < data->Devices[iDevice].ChannelData[iChannel].nFrames;
           iSample++)
...
void _WriteHeader(FILE* fp, sDataDescriptions* pBodyDefs) {
  ...
  if (!pBodyDefs->arrDataDescriptions[0].type == Descriptor_MarkerSet) return;
