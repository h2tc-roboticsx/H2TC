...
argparser.add_argument(
    "--fps_event", type=int, default=60, help="FPS for decoding event data into frames"
)
argparser.add_argument(
    "--fps_zed",
    type=int,
...
ZED_CAMS = {"rgbd0": "17471", "rgbd1": "24483054", "rgbd2": "28280967"}
...
def process(take, args):
    ...
    for stream_id in ["rgbd1"]:
        ...
        if not os.path.isfile(raw_path):
            raise Exception(
                "ZED {}: missing raw data at {}".format(stream_id, raw_path)
            )
        ...
        if os.path.exists(opt_path):
            ...
            zed.export_to_images(
                raw_path, opt_path, args.depth_img_format, args.depth_accuracy
            )
    ...
    # print("plotting hand pose and motion")
...
if __name__ == "__main__":
    ...
    # LX: realign to make sure no null
    for take in sorted(os.listdir(data_path), key=int):
        proc_dir = os.path.join(data_path, take, "processed")
        ts_paths = []
        ...
        ts_path = os.path.join(proc_dir, "event_frames_ts.csv")
        ts_paths.append(ts_path)
        ...
        for stream_id in [
            "sub1_head_motion",
            "sub2_head_motion",
            "sub1_right_hand_motion",
            "sub1_left_hand_motion",
        ]:
            proc_ts_path = os.path.join(proc_dir, "{}.csv".format(stream_id))
            opi_paths.append(proc_ts_path)
        # add timestamp file to the list for alignment
        ts_paths.append(opi_paths)

        hand_paths = []
        for stream_id in ["left_hand_pose", "right_hand_pose"]:
            proc_ts_path = os.path.join(proc_dir, "{}.csv".format(stream_id))
            hand_paths.append(proc_ts_path)
        ts_paths.append(hand_paths)
    ...
    #     process(take, args)
    #     time_end = time.time()
    #     print("processing cost", time_end - time_start, "s")
